...
DB.load = function() {
    ...
    alasql('CREATE TABLE emp(id INT IDENTITY, number STRING, name_kanji STRING, name_kana STRING, sex INT, birthday DATE, tel STRING, ctct_name STRING, ctct_addr STRING, ctct_tel STRING, pspt_no STRING, pspt_date STRING, pspt_name STRING, rental STRING, password STRING);');
    ...
    alasql('CREATE TABLE department(id INT IDENTITY, emp INT, department INT);');
    var pdepartments = alasql.promise('SELECT MATRIX * FROM CSV("data/DEPARTMENT-DEPARTMENT.csv", {headers: true})').then(
    ...
    // リロード
    Promise.all([ pemp, paddr, pfamily, pedu, pchoice, pdepartments]).then(function() {
